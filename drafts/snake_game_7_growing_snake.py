...
class SnakeFood(object):
    ...
    def draw(self):
        ...
        if self.shape=='square':
            x1,y1=self.position[0]*square_width,self.position[1]*square_height
            pygame.draw.rect(win,self.color,(x1,y1,self.size,self.size),0)
        elif self.shape=='circle':
            x1,y1=int(square_width*self.position[0]+0.5*self.size),int(square_height*self.position[1]-0.5*self.size)
